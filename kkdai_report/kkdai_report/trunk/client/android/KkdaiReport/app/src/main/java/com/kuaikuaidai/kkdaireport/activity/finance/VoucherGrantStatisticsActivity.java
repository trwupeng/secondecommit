...
public class VoucherGrantStatisticsActivity extends BaseFinanceActivity implements AdapterView.OnItemClickListener, PullToRefreshBase.OnRefreshListener2<ListView> {
    ...
    private QuickAdapter<VoucherGrant> adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_voucher_grant);
        ButterKnife.bind(this);
        init();
    }

    private void init() {
        tvTitle.setText(R.string.voucher_grant);
        ...
        adapter = new QuickAdapter<VoucherGrant>(mContext, R.layout.item_voucher_grant, list) {
            @Override
            protected void convert(BaseAdapterHelper helper, VoucherGrant item) {
                helper.setText(R.id.tv_grant_date, item.getYmd());
                helper.setText(R.id.tv_dixian_grant_money, item.getDixian_grant_amount());
                helper.setText(R.id.tv_tixian_grant_money, item.getTixian_grant_amount());
                helper.setText(R.id.tv_jiaxi_grant_num, item.getJiaxi_grant_num());
                helper.setText(R.id.tv_faxian_grant_money, item.getFanxian_grant_amount());
            }
        };
    ...
    private void getData() {
        CommSender.voucherGrantStatistics(startData, endData, String.valueOf(pageId), this, mContext);
    }
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        CommSender.voucherGrantDetails(ymd.replace("-", ""), "1", this, mContext);
    }
    ...
    public void onCallback(long code, String msg, Exception e, String api, String useData) {
        switch (api) {
            case CommUrlConstant.VOUCHER_GRANT_STATIATICS:
                ...
                if (!TextUtils.isEmpty(voucherSum.toString(1))) {
                    tvSum.setText(voucherSum.toString(1));
                ...
                list = VoucherGrantParse.getInstance().getList();
                ...
                if (list != null && list.size() != 0) {
                    ...
                    VoucherGrantParse.getInstance().setList(null);
                } else {
                    adapter.clear();
                }
            ...
            case CommUrlConstant.VOUCHER_GRANT_DETAILS:
                if (VoucherGrantParse.getInstance().getVoucherGrantDetails() != null && VoucherGrantParse.getInstance().getVoucherGrantDetails().size() != 0) {
                    Intent intent = new Intent(mContext, VoucherGrantDetailActivity.class);
    ...
    protected void onDestroy() {
        super.onDestroy();
        VoucherGrantParse.getInstance().clear();
    }

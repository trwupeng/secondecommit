...
public class ServiceFeeStatisticsActivity extends BaseFinanceActivity implements AdapterView.OnItemClickListener, PullToRefreshBase.OnRefreshListener2<ListView> {
    ...
    private QuickAdapter<ServiceFee> adapter;
    private List<ServiceFee> list;
    ...
    private void init() {
        tvTitle.setText(R.string.service_fee);

        tvHederDate.setText(R.string.date);
        tvHederServiceFee.setText(R.string.service_fee);
        ...
        list = new ArrayList<ServiceFee>();
        adapter = new QuickAdapter<ServiceFee>(mContext, R.layout.item_service_fee, list) {
            @Override
            protected void convert(BaseAdapterHelper helper, ServiceFee item) {
                helper.setText(R.id.tv_date, item.getYmd());
                helper.setText(R.id.tv_fee, item.getBid_serviceFee());
            }
        };
    ...
    private void getData() {
        CommSender.serviceFeeStatistics(startData, endData, String.valueOf(pageId), this, mContext);
    }
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        CommSender.serviceFeeDetailStatistics(ymd, "1", this, mContext);
    }
    ...
    public void onCallback(long code, String msg, Exception e, String api, String useData) {
        switch (api) {
            case CommUrlConstant.SERVICE_FEE_STATIATICS:
                ...
                list = ServiceFeeParse.getInstance().getServiceFeeList();
                ...
                if (list != null && list.size() != 0) {
                    ...
                    ServiceFeeParse.getInstance().setServiceFeeList(null);
                } else {
                    adapter.clear();
                }
            ...
            case CommUrlConstant.SERVICE_FEE_DETAILS:
                if (ServiceFeeParse.getInstance().getSeviceFeeDetailList() != null && ServiceFeeParse.getInstance().getSeviceFeeDetailList().size() != 0) {
                    Intent intent = new Intent(mContext, ServiceFeeDetailActivity.class);
    ...
    protected void onDestroy() {
        super.onDestroy();
        ServiceFeeParse.getInstance().clear();
    }

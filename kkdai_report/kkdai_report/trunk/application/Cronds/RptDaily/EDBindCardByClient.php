...
namespace PrjCronds;
/**
 * php /var/www/licai_php/public/crond.php "__crond/run&task=RptDaily.EDBindCardByClient&ymdh=20160128"
 * Created by PhpStorm.
 * User: li.lianqi
 * Date: 2016/1/28 0028
 * Time: 下午 1:10
...
error_log('BindOkByClient sql:'.\Sooh\Db\Broker::lastCmd());
        ...
        $where = 'resultYmd='.$ymd.' and statusCode in(2,3,-4) '
            .'and tb_bankcard_final.userId not in (select userId from tb_bankcard_final where statusCode in(4,16) and resultYmd='.$ymd.') ';
        $result = $this->getResult($this->sqlBase($where));
...
//error_log('RegAndBindFailedByClient sql:'.\Sooh\Db\Broker::lastCmd());
    ...
    protected function sqlBase($where) {
        return 'select tb_bankcard_final.clientType,tb_user_final.copartnerId, count(distinct(tb_bankcard_final.userId)) as n '
              ...
              .' group by tb_bankcard_final.clientType,tb_user_final.copartnerId';
    }

...
namespace PrjCronds;
/**
 * php /var/www/licai_php/public/crond.php "__crond/run&task=RptDaily.EDStockingByClient&ymdh=20160126"
 *
 * Created by PhpStorm.
 * User: li.lianqi
 * Date: 2016/1/26 0026
...
class EDStockingByClient extends \Sooh\Base\Crond\Task{
...
error_log('################################'.$ymd.' '.__CLASS__);
        ...
        $rechargeDay = \Rpt\EvtDaily\ChargeAmount::getCopy('ChargeAmountByClient');
        $where = 'ymd='.$ymd.' and amount>0 ';
        $result = $this->getResult($this->sqlBase($where));
        if (!empty($result)) {
            foreach($result as $r) {
                $rechargeDay->add($r['n'], $r['clientType'], $r['copartnerId'], 0);
            }
...
error_log('recharege total add:'.$rechargeDay->totalAdd);
        ...
        $withdrawDay = \Rpt\EvtDaily\WithdrawAmount::getCopy('WithdrawAmountByClient');
        $where = ' amount<0 and finishYmd='.$ymd.' and orderStatus=39 ';
        $result = $this->getResult($this->sqlBase($where));
        if (!empty($result)) {
            foreach($result as $r){
                $withdrawDay->add(-$r['n'], $r['clientType'], $r['copartnerId'], 0);
            }
...
error_log('withdraw total add:'.$withdrawDay->totalAdd);
        ...
        $applyWithdrawDay = \Rpt\EvtDaily\ApplyWithdrawAmount::getCopy('ApplyWithdrawAmountByClient');
        $where = 'ymd='.$ymd.' and amount<0 ';
        $result = $this->getResult($this->sqlBase($where));
        if (!empty($result)) {
            foreach($result as $r) {
                $applyWithdrawDay->add(-$r['n'], $r['clientType'], $r['copartnerId'], 0);
            }
...
error_log('apply withdraw add:'.$applyWithdrawDay->totalAdd);
    ...
    protected function sqlBase ($where) {
        return 'select tb_recharges_final.clientType,sum(tb_recharges_final.amount)/100 as n,tb_user_final.copartnerId '
            ...
            .'group by tb_recharges_final.clientType,tb_user_final.copartnerId';

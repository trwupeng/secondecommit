...
//     protected $pageSizeEnum = array(20, 50, 100);
    ...
    protected $flgDisplay = ['否', '是'];
    protected $promotionWay = array('cpc'=>'cpc','cpa'=>'cpa','cpm'=>'cpm','cps'=>'cps');
...
// var_log($where , 'where>>>>>>>>>>>>');
       ...
       $fields= array('contractId', 'copartnerAbs','remarks','promotionWay','displayRule','displayPercent','ymdStart', 'ymdEnd', 'profitsPlan', 'profitsFix',  'notes', 'flgDisplay');
       if ($isDownloadExcel) {
       ...
       $headers= array('协议Id'=>80, '渠道名称'=>70, '渠道简称'=>80, '协议名称'=>70, '推广方式'=>40,'数据展示规则'=>70, '数据展示比例'=>70, '开始日期'=>70,
           '结束日期'=>70, '分成方案'=>40, '分成修正'=>40,  '备注'=>70, '展示数据给渠道'=>80);
    ...
    public function addnewAction() {
        ...
        $formEdit->addItem('contractId', form_def::factory('协议Id', '', form_def::text)/*->verifyInteger(1, 100000000000000000, 999999999999999999)*/)
                ...
                ->addItem('displayRule', form_def::factory('展示数据规则', '0', form_def::select)->initMore(new options_def(\Rpt\LimitRules::$displayRule)))
                ->addItem('displayPercent', form_def::factory('数据展示比例%（0-100）', 0, form_def::text, [], ['data-rule' => 'required,digits']))
                ...
                ->addItem('flgDisplay', form_def::factory('展示数据给渠道', 0, form_def::select))
        ...
        $formEdit->item('flgDisplay')->options = new options_def($this->flgDisplay);
        ...
        if ($formEdit->flgIsThisForm){
            try{
                ...
                // 字段检查先放这里了。
                if($fields['displayPercent'] >100 || $fields['displayPercent'] <0 ) {
                    return $this->returnError('展示比例请输入0-100的数字');
                }

    ...
    public function viewsimpleAction() {
		...
		if(!empty($authCode) && !empty($copartnerid)){
			...
			if(empty($r) || $r['authCode']!=$authCode){
				...
				$this->rs = $this->db->getRecords('tb_user_final', 'userId,realname,ymdReg,length(idCard) as flg,phone,contractId,yuebaoInvestmentTotal,dingqiInvestmentTotal,ymdFirstBuy,shelfIdFirstBuy,amountFirstBuy',
						['contractId'=>$contractIds,'ymdReg>'=>date('Ymd',time()-45*86400)],'rsort ymdReg');

...
class MBMessage extends \Sooh\DB\Base\KVObj {
    ...
    public static function addRecord($title,$content,$sendid,$receiverid, $userid,$type,$batch_type,$batchid,$perf_id,$reply_id=null,$dstid=[],$arg1=0,$arg2=0,$arg3=0,$arg4='',$arg5='',$arg6=null) {
    ...
    public static function transformData ($record, $show=false) {
        ...
        if($show) {
            ...
            $tmp['queryParamers']['perf_id']=$record['perf_id'];//目标i或者日志id
            $tmp['queryParamers']['reply_id']=$record['reply_id'];//回复的ID
            $tmp['queryParamers']['message_id']=$record['id'];//消息的ID
            $tmp['queryParamers']['flag']=$record['flag'];//标记是否已读
            $tmp['queryParamers']['batch_type']=$record['batch_type'];//标记日志还是目标
            $tmp['queryParamers']['message_type']=$record['type'];//标记消息类型
            $tmp['queryParamers']['message_sendid']=$record['sendid'];//标记消息发送者
            
            if($record['batch_type'] == \Prj\Data\MBPerfReply::targetType){
                $parseRet = \Prj\Data\MBPerfDst::parseTypeId($record['batchid']);
                $tmp['queryController']='perfdst';
                $tmp['queryBtnName']='查看目标';
                $tmp['data_id'] = 'plan_perfdst_index'; 
                ...
                $tmp['targetTab'] = 'list';
                //$tmp['perf_id']=$record['perf_id'];//目标i或者日志id
                $tmp['queryParamers']['perf_id']=$record['perf_id'];//目标i或者日志id
                $tmp['queryParamers']['reply_id']=$record['reply_id'];//回复的ID
                $tmp['queryParamers']['message_id']=$record['id'];//消息的ID
                $tmp['queryParamers']['flag']=$record['flag'];//标记是否已读
    ...
    public static function unreadNum($userid) {
    	...
    	$where = ['receiverid'=>$userid, 'flag'=>[0,2]];//新增flag=2也为未读消息
    	//$where = ['receiverid'=>$userid, 'flag'=>0];
    	return $model->db()->getRecordCount($model->tbname(), $where);
    }
}

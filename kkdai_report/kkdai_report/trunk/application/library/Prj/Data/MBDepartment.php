...
class MBDepartment  extends BaseFK {
    ...
    //根据一个部门id, 获取所有基层子部门id列表
    public static function getAllSameDeptIdByDeptId( $deptId ) 
    {
    	$all = self::getRecords( ['supId' => $deptId] );
    	if ( 0 == count($all) )
    	{
    		return [$deptId];
    	}
    	...
    	foreach( $all as $v )
    	{
    		$ret = array_merge( $ret, self::getAllSameDeptIdByDeptId($v['id']) );
    	}
    ...
    //获取上级部门的id
    public static function getParentDeptIdByDeptId( $deptId )
    {
    	$rs = self::getRecords( array('id' => $deptId) );
    	if ( 0 == count($rs) )
    	{
    		return false;
    	}

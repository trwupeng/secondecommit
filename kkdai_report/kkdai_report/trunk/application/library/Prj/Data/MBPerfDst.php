...
class MBPerfDst extends KVObj
{
     ...
     */
 public static function addData($settime,$staffname,$name, $content, $level, $type, $target_ji,$target_yue, $target_week,$process,$type_num,$date,$createUserId, $userId)
    {
        $typeNum = self::buildTypeNum($type, strtotime($date));
        
        if(($type_num)&&$type_num!=$typeNum){
            $typeNum=$type_num;
        }
    ...
    public static function getData($id)
    {
    	$model = self::getCopy($id);
    	$model->load();
    	if ( $model->exists() && 0 == $model->getField( 'del' ) )
    	{
    		return $model;
    	}
   ...
   public static function getpair ($type,$userid) {
        
        $model = self::getCopy('');
        $db = $model->db();
        $tb = $model->tbname();
        if($userid)return $db->getPair($tb, 'id', 'name',['type'=>$type,'userid'=>$userid,'del'=>0]);

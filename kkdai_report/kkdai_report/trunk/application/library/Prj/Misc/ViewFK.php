...
class ViewFK {
    ...
    protected $_editCallback = [ //编辑按钮点击的回调
		'before' => null,
		'after' => null,
		'enable' => null,
    ];
    protected $_htmlTableId = 'default_table_id'; //表的id
    ...
    public function setEditCallback( $before, $after, $enable )
    {
    	$this->_editCallback['before'] = $before;
    	$this->_editCallback['after'] = $after;
    	$this->_editCallback['enable'] = $enable;
    }
    
    public function setHtmlTableId( $id )
    {
    	$this->_htmlTableId = $id;
    }
    
    public function useDefault( $default )
    {
    	$this->_useDefault = $default;
    }
     ...
     * @return $this
     */
    public function addRow($key , $name , $mark = null , $type = 'text' , $options = [] , $width = 300 , $setting = []){
        $tmp = [
            'name' => $name,
            'mark' => $mark,
            'type' => $type,
    ...
    public function addBtn($html, $showCond = null){
        $this->otBtn[] = $html;
        if ( $showCond )
        {
        	$this->otBtnCond[$html] = $showCond;
        }
    }
    ...
    protected function _input($key){
        ...
        switch ($type) {
            ...
            case 'select' :
            	$prefix = '';
            	if ( $this->_header[$key]['setting']['prefix'] )
            	{
            		$prefix = $this->_header[$key]['setting']['prefix'];
            	}
                $str = '<select onchange="map(this)" '
                		. 'onclick="map(this)" '
                		. 'class="'.$mark.'" '
                		. 'name="' . $prefix . 'values[#index#]['.$key.']" '
                		. 'data-toggle="selectpicker" '
                		. 'data-options="{id:\'values[#index#][ul_'.$key.'_ul]\'}" ';
                		if ( $this->_header[$key]['setting']['usercall'] )
                		{
                			$str .= 'usercall="' . $this->_header[$key]['setting']['usercall'] . '" ';
                		}
                		$str .= '>';
                foreach($options as $k=>$v){
...
html;
    ...
    protected function _headHtml($type = false){
        ...
        $this->_width = $width;
        $btn = !$this->show_action||$type ? '' : '<a href="javascript:;" class="btn btn-green" data-toggle="dosave">保存</a>&nbsp<a href="'.$deleteApi.'" class="btn btn-red row-del">删</a>';
        $headStr .= '<th title="" data-addtool="true" width="100">'.($this->show_action&&$this->table_type=='normal' ? '操作' : '').
            $btn.
            '</th>';
        return $headStr;
    }
    ...
    protected function _headPerfHtml($type = false){
        ...
        if ($this->operate) {
            $btn = !$this->show_action||$type ? '' : '<a href="javascript:;" class="btn btn-green" data-toggle="dosave">保存</a>&nbsp<a href="'.$deleteApi.'" class="btn btn-red row-del">删</a>';
            $headStr .= '<th title="" data-addtool="true" width="100">'.($this->show_action&&$this->table_type=='normal' ? '操作' : ''). $btn. '</th>';
        }
        return $headStr;
    }
     ...
     * 解析单元格
     */
    protected function _parseDatagrid( $data )
    {
    	$ret = [];
    	if ( is_array($data) )
    	{   
    		$ret['value'] = $data['value'] ? $data['value'] : $data;
    		$ret['type'] = $data['type'] ? $data['type'] : 'text';
    		$ret['style'] = $data['style'] ? $data['style'] : '';
    		$ret['title'] = $data['title'] ? $data['title'] : $ret['value'];
    		$ret['toggle'] = $data['toggle'] ? $data['toggle'] : '';
    		$ret['alert'] = $data['alert'] ? $data['alert'] : '';
    		$ret['url'] = $data['url'] ? $data['url'] : '';
    		$ret['percent'] = $data['percent'] ? intval($data['percent']) : 0;
    		$ret['prefix'] = $data['prefix'] ? $data['prefix'] : '';
    	}
    	else
    	{
    		$ret['type'] = 'text';
        ...
        foreach($this->_data as $k=>$v){
            ...
            foreach($this->_header as $key=>$arr){
               ...
               /*
                ...
                if ( is_array($v[$key]) )
                {
                	$value = $v[$key]['value'] ? $v[$key]['value'] : $v[$key];
                	$style = $v[$key]['style'] ? $v[$key]['style'] : '';
                	$title = $v[$key]['title'] ? $v[$key]['title'] : $v[$key];
                	$toggle = $v[$key]['toggle'] ? $v[$key]['toggle'] : '';
                	$alert = $v[$key]['alert'] ? $v[$key]['alert'] : '';
                	$url = $v[$key]['url'] ? $v[$key]['url'] : '';
                }
                else
                {
                	$value = $v[$key];
               ...
               */
                ...
                if(is_array($ret['value'])){
                    $ret['value'] = implode(',',$ret['value']);
                }else{
                    if($arr['options']&&$useDefault)$ret['title'] = $arr['options'][$ret['value']];
                }
                $class = $num < 2 ? "floatBtn" : "";
                if ( strlen($url) == 0 )
                {
                /*
                	$tmp .= '<td>' 
                			. '<div class="processbg">' 
                					. '<div id="processbar" class="processfg"/>'
                					. '<div class="processtext>10%</div>'            					
                			. '</div>'                			
                		. '</td>';
                */
                	if ( 'process' == $ret['type'] )
                	{
                		$tmp .= '<td class="' . $class . '">' . View::processDefaultInDatagrid( $ret['prefix'] . $idVal, $ret['percent'], $ret['value']) . '</td>';
                	}
                	else
                	{
                		$tmp.='<td class="'.$class
                ...
                {
                	if ( strlen($ret['alert']) > 0 )
                	{
                		$tmp .= '<td><a href="'.$ret['url']
                			 .'" class="btn btn-default" style="border:none" data-toggle="doajax">' . $ret['value'] . '</a></tb>';
                	}
                	else 
                	{
                		$tmp .= '<td><a type="button" class="btn btn-green" style="border:none" '
                			 . 'data-toggle="navtab" data-id="'.$ret['title']
                		 	. '"  data-url="'.$ret['url']
                		 	. '" data-title="'.$ret['title'].'">'.$ret['value'].'</a></td>';
                	}
                }
//                $tmp.='<td class="'.$class.'" style="'.$style.'" data-toggle="' . $toggle . '" data-url="' . $url . '" data-key="'.$key.'" data-val="'.$dataValue.'" title="'.$title.'">'.$value.'</td>';
//                var_log( $tmp, 'tmp' );
                $num++;
            }
            $otBtn = '';
...
//            	var_log( $v, '$v' );
				$show = true;
				if ( isset( $this->otBtnCond[$v] ) )
				{
					if ( 0 == $oneRecord[$this->otBtnCond[$v]] )
					{
						$show = false;
					}
				}
...
//                var_log( $ret, 'ret' );
               ...
               */
                ...
                {
                	$tmp.='<td class="'.$class.'" style="'.$ret['style'].'" data-key="'.$key.'" data-val="'.$ret['value'].'" title="'.$ret['title'].'">'.$ret['title'].'</td>';
                }
//                var_log( $tmp, 'tmp' );
                $num++;
            }
...
html;
    ...
    public function show(){
        ...
        $tableType = 'tabledit';
		if ( null != $this->extHeader ) {
			$search = $this->extHeader;
		}
		$callback = '';
		if ( $this->_editCallback['before'] )
		{
			$callback .= $this->_editCallback['before'];
		}
		else
		{
			$callback .= 'null';
		...
		$callback .= ',';
		if ( $this->_editCallback['after'] )
		{
			$callback .= $this->_editCallback['after'];
		}
		else
		{
			$callback .= 'null';
		...
		$callback .= ',';
		if ( $this->_editCallback['enable'] )
		{
			$callback .= $this->_editCallback['enable'];
		}
		else
		{
			$callback .= 'null';
        ...
        <td style="vertical-align: top;position: relative" id="$mainTable">
            ...
            <div style='$tableStyle' class="zhuyaoCon">
                <form  data-toggle="validate" method="post" data-id="{$this->_tableId}">
                    <table id="$htmlTableId" style="width: {$this->_width}px;min-width: 100%" class="table table-bordered table-hover table-striped table-top table-$pid" data-toggle="$tableType" data-initnum="0" data-action="$updateApi" data-callback="$callback" data-single-noindex="true">
                        <thead>
...
html;
    ...
    public function normalTable(){
    	$viewType = \Sooh\Base\Ini::getInstance()->get( 'viewRenderType' );
    	if ( $viewType == 'json' || $viewType == 'jsonp' )
    	{
    		return ['header'=>$this->_header, 'data'=>$this->_data];
    	}
    	else
    	{
	...
	public function setShowAction( $show ) {
		$this->show_action = $show;
	}

	public function setExtHeader( $header ) {
		$this->extHeader = $header; 
	}

	public function showExtHeader() {

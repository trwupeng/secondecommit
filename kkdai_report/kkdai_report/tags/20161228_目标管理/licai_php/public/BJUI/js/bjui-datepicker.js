...
+function ($) {
    ...
    var $box, $main, $prev, $next, $year, $month, $time, $timeinps, $spinner, $hh, $mm, $ss, $tm, $close, $days, $dayNames, $clearBtn, $okBtn , $foot
    ...
    Datepicker.prototype.TOOLS = function() {
        ...
        var tools = {
            ...
            closeCalendar: function(flag) {
                if(that.options.mb_fix == true)return
                tools.changeTmMenu()
                if (flag) {
            ...
            selectWeek : function (arr) {
                $box.find('.mb-cal-isset').remove()
                for(var i in arr){
                    $box.find('[data-mb-week="'+arr[i]+'"][data-mb-day="1"]').prepend("<div class='mb-cal-isset'>已设置</div>")
                }
            },
            selectDay : function (arr) {
                $box.find('.mb-cal-isset-day').remove()
                for(var i in arr){
                    $box.find('[data-day-in="'+arr[i]+'"]').prepend("<div class='mb-cal-isset-day'><div class='point'></div></div>")
                }
            }
        }
        return tools
    ...
    /*
    Datepicker.prototype.mb_week_select = function (dw) {
        if(this.options.mb_week_back && this.options.mb_week_back_format){
            var that = this
            console.log('week select ...')
            console.log(dw)
            window[this.options.mb_week_back](dw , function (data) {
                var arr = window[that.options.mb_week_back_format](data)
                that.TOOLS().selectWeek(arr)
            })
        }
    }
    */
    Datepicker.prototype.mb_select = function (dw , type) {
        var back = 'mb_'+type+'_back'
        var format = 'mb_'+type+'_back_format'
        if(this.options[back] && this.options[format]){
            var that = this
            console.log('week select ...')
            console.log(dw)
            window[this.options[back]](dw , function (data) {
                var arr = window[that.options[format]](data)
                if(type == 'week'){
                    that.TOOLS().selectWeek(arr)
                }else if(type == 'day'){
                    that.TOOLS().selectDay(arr)
                }
            })
        }
    }
    ...
    Datepicker.prototype.init = function() {
        ...
        $year.add($month).off(this.events.change_ym).on(this.events.change_ym, function() {
            ...
            that.mb_select(tools.getDateWrap(date) , 'week')
            that.mb_select(tools.getDateWrap(date) , 'day')
        })
        ...
        //by tgh 20161223
        if(this.options.mb_fix){
            $close.hide()
            $foot.hide()
            this.mb_select(dw , 'week')
            this.mb_select(dw , 'day')
            //$box.css('position','relative')
            that.$element.html($box)
            $box.css('left',0)
            $box.css('top',0)
        }
    }
    ...
    Datepicker.prototype.create = function(dw, minDate, maxDate) {
        ...
        if (startDay > 0) {
            ...
            for (var t = prevDateWrap.days - startDay + 1; t <= prevDateWrap.days; t++) {
                ...
                dayStr += '<dd data-mb-day="'+week%7+'" data-mb-week="'+Math.floor(week/7)+'" class="other'+ _ctrClass +'" data-month="-1" data-day="'+ t +'">'+ t +'</dd>'
                week ++;
            }
        }
        for (var t = 1; t <= dw.days; t++) {
            ...
            if (t == dw.day)
                _ctrClass += ' slt'
            dayStr += '<dd data-day-in="'+ t +'" data-mb-day="'+week%7+'" data-mb-week="'+Math.floor(week/7)+'" class="'+ _ctrClass +'" data-day="'+ t +'">'+ t +'</dd>'
            week ++;
        }

...
if(is_numeric($uid)) {
	...
	include_once(UC_ROOT . '../config/config_ucenter.php');
	...
	$_db = new ucserver_db();
	$_db->connect(UC_DBHOST, UC_DBUSER, UC_DBPW, UC_DBNAME, UC_DBCHARSET, UC_DBCONNECT, UC_DBTABLEPRE);

	$avatar = $_db->fetch_first("SELECT url FROM " . UC_DBTABLEPRE . "avatar WHERE uid = $uid");
	$avatar = $avatar['url'];

	//$avatar = $uid ? C::t('ucenter_avatar')->fetch_by_uid($uid, 'url') : '';

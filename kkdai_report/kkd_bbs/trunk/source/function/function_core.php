...
//===========================code start==================================
function checkbbsnickname($kkduid)
{
    $ret = C::t('common_member_nickname')->fetch_by_uid($kkduid);
    if ($ret !== '') {
        return $ret;
    }else{
        return false;
...
function curl_post($url,$post=array(),$header = [] ,$time=5 )
{
    ...
    if(empty($header['Cookie']))curl_setopt($ch, CURLOPT_COOKIEFILE, $cookie_file); //使用上面获取的cookies
    ...
    //var_log(curl_getinfo($ch),'curl_info');
    $return = curl_exec ( $ch );
    ...
    curl_close ( $ch );
    if($returnArr = json_decode($return,true)){
        return $returnArr;
    }else{
        return $return;
    }
 ...
 * @param $num
 */
function changeKlb($uid,$kkdId,$num ){
    $sn = time().sprintf("%07d", $uid).mt_rand('1000','9999');
    $db = new discuz_table(['table'=>'a_klb_log','pk'=>'sn']);
    $db->insert([
        'sn'=>$sn,
        'time'=>date('YmdHis'),
        'uid'=>$uid,
...
function decKlb($num){
    ...
    global $_G;
    $db = new discuz_table(['table'=>'common_member_nickname','pk'=>'bbsId']);
    $res = $db->fetch($_G['uid']);
    $token = $res['token'];
    $kkdId = $res['uid'];
    $data = [
    ...
    if(checkmobile()===false){
        //PC端
        unset($data['token']);
        $api = $_G['config']['kkd']['bbsComment']['web'];
        if(empty($api))return showmessage('系统错误,缺少decCountUrl配置');
        $url = $api.'?'.http_build_query($data);
        $ret = curl_post($url,[],['Cookie'=>'JSESSIONID='.$_COOKIE['JSESSIONID']],5);
    }else{
        //H5端
        if(empty($token))logOut('系统错误,无法获取token');
        $api = $_G['config']['kkd']['bbsComment']['h5'];
        if(empty($api))return showmessage('系统错误,缺少decCountUrl配置');
        var_log('###token:'.$token);
        $url = $api.'?'.http_build_query($data);
        $ret = curl_post($url,[],[],5);
    ...
    var_log($ret,'###'.$url);
    if($ret['code'] === null)return showmessage('系统正忙,请稍后重试');
    //return $model->showmessage('手机强制中断嘿嘿');
    switch($ret['code']){
        case 0 : break;
        case -2 :
            clearcookies();
            return showmessage('登录过期,请重新登录');
        case -4 :
            return showmessage($ret['desc']);
        default :
            return showmessage('系统正忙,请稍后重试');
    }
    changeKlb($_G['uid'],$kkdId,$data['coinCount']);
}

function getKkdname($uid , $all = false){
    $db = new discuz_table(['table'=>"common_member_nickname",'pk'=>'bbsId']);
    $ret = $db->fetch($uid);
    return $all ? $ret : $ret['kkdname'];
}

function getKkdnameByUname($username){
    $db = new discuz_table(['table'=>"common_member_nickname",'pk'=>'nickname']);
    $ret = $db->fetch($username);
    return $ret['kkdname'];
}
...
function getuserbyuid($uid, $fetch_archive = 0) {
    ...
    if($users[$uid]){
        $users[$uid]['kkdname'] = getKkdname($uid);
    }
	return $users[$uid];
}
...
function dexit($message = '') {
    if(function_exists('var_log'))var_log('===================[end]'.$_SERVER['REMOTE_ADDR'].'======================'."");
	echo $message;
	output();
	exit();
}
 ...
 */
function ajaxReturn($data = [], $msg = 'success', $code = '200', $type = 'json', $callback = 'callback')
{
    $ret = [
        'code' => $code,
        'msg' => $msg,

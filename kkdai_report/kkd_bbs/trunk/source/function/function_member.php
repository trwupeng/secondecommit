...
function logOut($str = null){
    global $_G;
    //require_once libfile('class/member');
    clearcookies();
    if(defined('IN_MOBILE')) {
        showmessage($str?$str:'location_logout_succeed_mobile', dreferer(), array('formhash' => FORMHASH, 'referer' => rawurlencode(dreferer())));
    } else {
        showmessage($str?$str:'logout_succeed', dreferer(), array('formhash' => FORMHASH, 'ucsynlogout' => '', 'referer' => rawurlencode(dreferer())));
    }
}
...
function clearcookies() {
    ...
    setcookie('token','');
    setcookie('JSESSIONID','',0,null,'kuaikuaidai.com');
	$_G['uid'] = $_G['adminid'] = 0;

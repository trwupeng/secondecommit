	...
	if(TIMESTAMP - $_G['cache']['grids']['cachetime'] < $cachelife) {
		...
		foreach($grids as $type => $gridthreads) {
			foreach($gridthreads as $key => $gridthread) {
                $gridthread['datelinee'] = $gridthread['dateline'];
                $gridthread['lastpostt'] = $gridthread['lastpost'];
...
if(!$gid && (!defined('FORUM_INDEX_PAGE_MEMORY') || !FORUM_INDEX_PAGE_MEMORY)) {
    ...
    $lyqAnnouncements = lyq_get_announcements();
...
//============== tgh strat ========================================================================
//需要开启首页四格,设置配置文件 $_G['config']['ext']['newsFid']
$beginTime = microtime(true);
list($threadsJinghuaList,$threadsHangyeList,$forumViews) = tgh_get_homeData();
$replys = $posts - $threads;
var_log("今日: $todayposts|昨日: {$postdata['0']}|帖子: $threads |回复数: $replys");
var_log('耗时'.(floor((microtime(true) - $beginTime)*1000)).'毫秒!');
/**
 * 获取首页需要的数据
 * [$threadsJinghuaList,$threadsHangyeList,$forumViews]
 * @return array
 */
function tgh_get_homeData(){
    /**
     * @var table_forum_thread $forumTb
     * @var table_forum_post $postsTb
     */
    GLOBAL $_G;
    GLOBAL $forumlist;
    ...
    $pageSize = 8;
    $cache = 1; //开启缓存
    $cacheLong = 3600; //缓存周期s

    $forumTb = C::t('forum_thread');
    $homeCacheData = [];
    $newsFid = $_G['config']['ext']['newsFid'] ? $_G['config']['ext']['newsFid'] : array_keys($forumlist)[0];
    var_log('行业资讯ID:'.$newsFid);
    require_once libfile('function/cache');
    if($cache && !$_GET['_cacheRefresh']){
        if(file_exists(getcachepath('homeCacheData'))){
            require_once(getcachepath('homeCacheData'));
        }
        if(empty($homeCacheData) || time() - $homeCacheData['time'] > $cacheLong){
            $homeCacheData = [];
        }
    }
    if(empty($homeCacheData)){
        //获取精华
        $threadsJinghuaList = $forumTb->fetch_all_by_digest_displayorder(3,'=',0,'>=',0,$pageSize);
        //获取行业资讯
        $threadsHangyeList = $forumTb->fetch_all_by_fid_displayorder($newsFid - 0,0,null,null,0,$pageSize);
        $forumViews = [];
        if(is_array($forumlist)){
            foreach($forumlist as $forum){
                $threadList = $forumTb->fetch_all_by_fid_displayorder($forum['fid'],0);
                $views = 0;
                if(is_array($threadList)){
                    //var_log($threadList,'帖子列表>>>');
                    array_walk($threadList,function($v,$k)use(&$views){
                        $views+=$v['views'];
                    });
                }
                $forumViews[$forum['fid']] = $views;
            }
        }
        $homeCacheData = [
            'threadsJinghuaList' => $threadsJinghuaList,
            'threadsHangyeList' => $threadsHangyeList,
            'forumViews' => $forumViews,
            'time' => time()
        ];
        if($cache)writetocache('homeCacheData',getcachevars(['homeCacheData'=>$homeCacheData]));
    }else{
        var_log('读取缓存=>homeCacheData');
    }
    return array_values($homeCacheData);
}
//=============================== tgh end ===========================================================

//==============lyq strat=================
//获取广告位1,2,3
$advertise1 = lyq_get_advertise_by_name('a1');
$advertise2 = lyq_get_advertise_by_name('a2');
$advertise3 = lyq_get_advertise_by_name('a3');
$advertise4 = lyq_get_advertise_by_name('a4');
$advertise5 = lyq_get_advertise_by_name('a5');
$advertise6 = lyq_get_advertise_by_name('a6');
$advertise7 = lyq_get_advertise_by_name('a7');
$advertise8 = lyq_get_advertise_by_name('a8');
$advertiseCount = function ($arrAd) {
    $count = 0;
    foreach ($arrAd as $v) {
        if ($v['available'] == 1) {
            $count++;
...
list($kkdRankInvestment, $kkdRankInvite, $userRanking) = lyq_get_ranking();

$isForumIndex = 1;
if ($_G['member']['username']) {
    $_G['member']['lyqavatar'] = lyq_get_onlyavatar_url($_G['member']['uid']);
}
//==============lyq end=================
 ...
 */
function lyq_get_advertise_by_name($name)
{
    $advertise = C::t('common_advertisement')->fetch_all_search($name, '', '', '', '', '', 0, 1);
    if (!empty($advertise)) {
        $advertise = $advertise[0];
        $parameters = unserialize($advertise['parameters']);
        unset($advertise['parameters']);
        $ret = array_merge($parameters, $advertise);
...
function lyq_get_announcements() {
    global $_G;
    if($_G['cache']['announcements']) {
        return $_G['cache']['announcements'];
    }

    return [];
}

function lyq_get_ranking()
{
    $kkdRankInvestment = $kkdRankInvite = [];
    require_once(getcachepath('kkdRankInvestment'));
    require_once(getcachepath('kkdRankInvite'));

    $kkdRankInvestment = array_slice($kkdRankInvestment, 0, 8);
    $kkdRankInvite = array_slice($kkdRankInvite, 0, 8);

    $admingroup = C::t('common_admingroup')->fetch_all_merge_usergroup();
    $excludeSql = ' WHERE m.groupid NOT IN (' . implode(array_keys($admingroup), ',') . ')';
    $userRanking = C::t('common_member')->fetch_all_order_by_credit_for_ranklist_exclude_admingroup(8, $excludeSql);
    
    foreach ($userRanking as $k => $v) {
        $userRanking[$k]['imageUrl'] = lyq_get_onlyavatar_url($v['uid']);
        //替换快快贷昵称
        $kkdname = getKkdname($userRanking[$k]['uid']);
        $v['username'] = $kkdname?$kkdname:$userRanking[$k]['username'];
        $userRanking[$k]['username'] = lyq_cut_username($v['username']);

    }
    return [$kkdRankInvestment, $kkdRankInvite, $userRanking];
}

function lyq_cut_username($username)
{
    if (function_exists('mb_strwidth')) {
        if (mb_strlen($username, 'utf8') > 3) {
            return mb_substr($username, 0, 3, 'utf8') . '***';
//            return mb_strimwidth($username, 0, 6, '***', 'utf8');
        }
    }
    return $username;
}
 ...
 * @author lingtm <605415184@qq.com>
 */
function lyq_get_onlyavatar_url($uid, $size = 'small')
{
    global $_G;

...
if($_GET['from'] == 'portal' && !$_G['setting']['portalstatus']) {
	...
	$_GET['from'] = '';
} elseif($_GET['from'] == 'album' && ($_G['setting']['guestviewthumb']['flag'] && !$_G['uid'] && IN_MOBILE != 2 || 1==2)) { //!$_G['group']['allowgetimage'] by tgh 取消路人不允许查看图片
	$_GET['from'] = '';
}
...
foreach($postarr as $post) {
	if(($onlyauthoradd && empty($post['anonymous']) || !$onlyauthoradd) && !isset($postlist[$post['pid']])) {
        ...
        //帖子名字搞一搞
        $kkdName = getKkdname($post['authorid']);
        $post['author'] = $kkdName ? $kkdName : $post['author'];

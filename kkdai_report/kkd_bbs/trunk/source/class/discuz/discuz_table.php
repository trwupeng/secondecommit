...
class discuz_table extends discuz_base
{
    ...
    //=========================code by tgh======================================
    public function getRecords($where = [],$sort = '',$start = 0,$limit = 0 , $tableid = null){
        return DB::fetch_all('SELECT * FROM %t WHERE '.DB::parseWhere($where).' '.($sort?'order by '.$sort:'').' '.($limit||$start ? 'limit '.$start.($limit?','.$limit:'') : ''), array($this->_table));
    }

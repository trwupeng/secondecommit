...
class discuz_database {
    ...
    public static function parseWhere($where = []){
        ...
        if(is_array($where)){
            foreach($where as $k=>$v){
                $tail = substr($k,-1);
                $glue = self::$whereMap[$tail];
                if(!empty($glue))$k = substr_replace($k,'',-1);
                if(is_array($v)){
                    $glue = $glue=='<>' ? 'notin' : 'in';
                }else{
                    $glue = $glue?$glue:'=';
                }
                $whereStr .= ' AND '.self::field($k,$v,$glue);
            }
        }else{
            $whereStr = $where;
        }
    ...
    public static function lastCmd($num = 5){
        return array_slice(self::$sqls,-1*$num,$num);
    }

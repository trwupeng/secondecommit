...
//====================自定义日志 by tgh=========================
defined(DIR) or define('DIR',DISCUZ_ROOT);
defined(PID) or define('PID',mt_rand(1000,9999));
...
if(date('d')%3 == 0 && !file_exists($logFile)){
    var_log('执行日志清理任务');
    $fileNames = scandir($logDir);
    array_walk($fileNames,function($v,$k) use ($logDir,$logLine){
        if(preg_match('/^error_log_[0-9]{8}\.log$/',$v)){
            $logDate = str_replace(['error_log_','.log'],'',$v);
            if(strtotime(date('Ymd')) - strtotime($logDate) > $logLine * 86400){
                unlink($logDir.'/'.$v);
                var_log('处理日志文件:'.$v);
            }
        }
...
function var_log($value,$tag = ''){
    $tag = empty($tag)?'':$tag."\n";
    $tag = '['.PID.']'.$tag;
    if(ERRORLOG){
        error_log($tag.var_export($value,true));
    }
}
//------------------------------------------------------

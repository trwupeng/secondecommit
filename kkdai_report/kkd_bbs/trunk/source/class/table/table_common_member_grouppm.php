...
class table_common_member_grouppm extends discuz_table
{
    ...
    public function fetch_all_by_uid_limit($uid, $type, $start, $limit)
    {
        return $uid ? DB::fetch_all('SELECT * FROM %t WHERE uid=%d AND `status`'.($type ? '>=' : '=').'0' . DB::limit($start, $limit), array($this->_table, $uid), 'gpmid') : array();
    }

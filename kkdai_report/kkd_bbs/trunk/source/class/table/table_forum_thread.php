		...
		if(($data = $this->fetch_cache($tids)) === false || count($tids) != count($data)) {
			...
			if(!empty($tids)) {
				...
				while($value = DB::fetch($query)) {
                    //带入快快贷昵称
                    if(function_exists('getKkdname')){
                        $kkdname = getKkdname($value['authorid']);
                        $value['author'] = $kkdname?$kkdname:$value['author'];
                    }
        ...
        if(function_exists('getKkdname')){
            if($data){
                array_walk($data,function(&$v,$k){
                    $kkdname = getKkdname($v['authorid']);
                    $v['authorKkdname'] = $kkdname?$kkdname:$v['author'];
                    $v['author'] = $v['authorKkdname'];
                    if(function_exists('getKkdnameByUname')){
                        $lastposterKkdname = getKkdnameByUname($v['lastposter']);
                        $v['lastposterKkdname'] = $lastposterKkdname?$lastposterKkdname:$v['lastposter'];
                    }
                });
            }
        }

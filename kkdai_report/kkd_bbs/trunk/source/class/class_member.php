...
class logging_ctl {
	...
	function on_login() {
		...
		if(!submitcheck('loginsubmit', 1, $seccodestatus)) {
            ...
            function doLogin(){
                global $_G;
                $api = $_G['config']['kkd']['loginApiH5'];
                $url = $api.'?'.http_build_query(['username'=>$_GET['username'],'password'=>$_GET['password'],'callback'=>'']);
                $data = curl_post($url);
                ...
                if($ret){
                    if($ret['code'] === 0){
                        $kkdId = $ret['customerId'];
                        //$nickname = $ret['data']['nickname'];
                        $img = $ret['data']['imageUrl'];
                        $syncRet = discuz_application::kkdSync($kkdId,$img,$_SERVER['REMOTE_ADDR'] ,$kkdId ,'' ,$ret['token'] ,$ret['data']['phone'] , $ret['data']['userName']);
                        $_GET['username'] = $syncRet['username'];
                        $_GET['password'] = $syncRet['pwd'];
                        return [
                            'kkdname' => $ret['data']['userName'],
                            'kkdphone' => $ret['data']['phone']
                        ];
                    }else{
                        showmessage($ret['desc']);
                    }
                }
            }
            $kkdInfo = doLogin();
			...
			if($result['status'] > 0) {
				...
				setloginstatus($result['member'], $_GET['cookietime'] ? 2592000 : 0); //设置登录状态
				checkfollowfeed();
				...
				if(empty($_GET['handlekey']) || !empty($_GET['lssubmit'])) {
                    ...
                    $param['username'] = $kkdInfo['kkdname'];
					showmessage($loginmessage, $location, $param, $extra);
				}
			} else {

...
var popup = {
	init : function() {
		var $this = this;
		jQuery('.popup').each(function(index, obj) {
			obj = jQuery(obj);
			var pop = jQuery(obj.attr('href'));
	...
	maskinit : function() {
		var $this = this;
		jQuery('#mask').off().on('tap', function() {
			$this.close();
		});
	},
	...
	open : function(pop, type, url) {
		...
		if(typeof pop == 'string') {
			...
			jQuery('body').append('<div id="ntcmsg" style="display:none;">'+ pop +'</div>');
			pop = jQuery('#ntcmsg');
		}
		if(POPMENU[pop.attr('id')]) {
			jQuery('#' + pop.attr('id') + '_popmenu').html(pop.html()).css({'height':pop.height()+'px', 'width':pop.width()+'px'});
		} else {
			pop.parent().append('<div class="dialogbox" id="'+ pop.attr('id') +'_popmenu" style="height:'+ pop.height() +'px;width:'+ pop.width() +'px;">'+ pop.html() +'</div>');
		}
		var popupobj = jQuery('#' + pop.attr('id') + '_popmenu');
		...
		jQuery('#mask').css({'display':'block','width':'100%','height':'100%','position':'fixed','top':'0','left':'0','background':'black','opacity':'0.2','z-index':'100'});
		POPMENU[pop.attr('id')] = pop;
	},
	close : function() {
		jQuery('#mask').css('display', 'none');
		$.each(POPMENU, function(index, obj) {
			jQuery('#' + index + '_popmenu').css('display','none');
		});
	}
};
...
var dialog = {
	init : function() {
		jQuery(document).on('click', '.dialog', function() {
			var obj = jQuery(this);
...
var formdialog = {
	init : function() {
		jQuery(document).on('click', '.formdialog', function() {
			popup.open('<img src="' + IMGDIR + '/imageloading.gif">');
			var obj = jQuery(this);
...
var display = {
	init : function() {
		var $this = this;
		jQuery('.display').each(function(index, obj) {
			obj = jQuery(obj);
			var dis = jQuery(obj.attr('href'));
			if(dis && dis.attr('display')) {
				...
				obj.on('click', function(e) {
					...
					if(dis.attr('display') == 'true') {
						...
						jQuery('#mask').css({'display':'block','width':'100%','height':'100%','position':'fixed','top':'0','left':'0','background':'transparent','z-index':'100'});
					}
					return false;
				});
			}
	...
	maskinit : function() {
		var $this = this;
		jQuery('#mask').off().on('touchstart', function() {
			$this.hide();
		});
	},
	hide : function() {
...
var pullrefresh = {
	init : function() {
		...
		.on('touchmove', function(e) {
			...
			if(!status && jQuery(window).scrollTop() <= 0) {
				status = true;
...
jQuery(document).ready(function() {
	...
	if(jQuery('div.pg').length > 0) {
		page.converthtml();
	}
	if(jQuery('.scrolltop').length > 0) {
		scrolltop.init(jQuery('.scrolltop'));
	}
	...
	if(jQuery('.display').length > 0) {
		display.init();
	}
	if(jQuery('.atap').length > 0) {
		atap.init();
	}
	if(jQuery('.pullrefresh').length > 0) {
		pullrefresh.init();
	}
	dialog.init();
	formdialog.init();
...
var alertFK = {
    ...
    showAlertFK : function(){
        var str = '<div class="alertFK" style="">'+
            '    <div style="margin-top: 20px;color: #919191">提示</div>'+
            '<div style="margin-top: 28px;font-size: 13px;text-align: left;width: 215px;margin-left: auto;margin-right: auto;color:black">您的昵称与账号相同，为保护您的个人隐私，建议您修改一个新的昵称后再进行发帖操作</div>'+
            ...
            '<style>'+
            '.alertFK{'+
            '    width: 75%;height: 240px;border: 1px solid;text-align: center;'+
            '    position: fixed;'+
...
function heiheiheiSubmit(obj){
    console.log('click...');
    if(alertFK.isShowAlert()){
        alertFK.showAlertFK();
        alertFK.nextStep = function(){
            $('#postsubmit').click();
        }
        return false;
    }else{
...
jQuery(function(){
    alertFK.init();
    $('#fastpostform').on('submit',function(e){
        console.log('submit...');
        if(alertFK.isShowAlert()){
            alertFK.showAlertFK();
            alertFK.nextStep = function(){
